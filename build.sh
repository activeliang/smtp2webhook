...
# local build 
docker build --platform linux/amd64 -t registry.cn-hongkong.aliyuncs.com/activeliang/smtp2webhook .
# docker tag ...
...
docker container rm -f smtp2webhook
mkdir -p /var/log/smtp2webhook
docker run -d -p 25:25  --name smtp2webhook -e PUSHOVER_TOKEN='ac63ykyumvestd1eahu8kveo9rngkp' -e PUSHOVER_USER='udgp4jdju8navenf9dy5nt3tncdgmt' --restart=always -v /var/log/smtp2webhook:/app/logs registry.cn-hongkong.aliyuncs.com/activeliang/smtp2webhook
